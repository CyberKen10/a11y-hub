...
| `knowledge_items` | Contenido unificado: título, resumen, contenido Markdown, `metadata` JSONB validada por tipo, estado y origen de importación (`source_sheet_*`). |
...
| `sync_jobs` | Trabajos idempotentes de `reindex` (embeddings del chat), con reintentos y último error. |
| `audit_logs` | Quién hizo qué y cuándo. |

## Decisiones clave

- **DB como fuente de verdad.** El contenido entra a mano, por Excel o por enlace de Google Sheet (solo lectura). Approaches también se puede sembrar con `npm run seed:approaches`. El origen queda en `source_sheet_*` con checksum para no duplicar filas. Sincronización solo reindexa el chat.
...
- **Prompt injection**: el sistema del chat marca el contenido recuperado como datos, no instrucciones; el texto de adjuntos y de importaciones se trata como no confiable.
